...
for key in runtime-token operator-token inference-token audit-token monitoring-token assignment-signing-secret catalog-signing-secret machine-commerce-signing-secret exclusive-lease-signing-secret credential-handoff-issuer-secret credential-issuer-adapter-token client-handoff-issuer-secret lease-helper-receipt-secret guide-diagnostics-token api-key-issuer-secret account-session-issuer-secret provider-callback-secret commercial-provider-adapter-token artifact-worker-callback-token artifact-scanner-callback-token entitlement-authority-callback-token guide-admin-auth-key guide-observation-secret; do
  rg -q "^  $key: " "$manifest"
done
...
test "$control_value_count" -eq 23
