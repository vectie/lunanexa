...
for key in runtime-token operator-token inference-token audit-token monitoring-token assignment-signing-secret catalog-signing-secret exclusive-lease-signing-secret credential-handoff-issuer-secret client-handoff-issuer-secret lease-helper-receipt-secret guide-diagnostics-token api-key-issuer-secret provider-callback-secret artifact-worker-callback-token artifact-scanner-callback-token entitlement-authority-callback-token guide-admin-auth-key guide-observation-secret; do
  rg -q "^  $key: " "$manifest"
done
...
test "$control_value_count" -eq 19
