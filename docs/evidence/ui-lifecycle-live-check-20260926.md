...
后续更新：本轮 ¥1,728 报价已过期，Spark 自助商品现价为 ¥49/天；见 [日价发布核对](spark-daily-price-live-20260926.md)。以下保留当时的按钮与状态证据。
...
7. 直接复用先前打开的 `:5003` MoonCode 标签发消息，页面显示 `Invalid character 'L' at line 1, column 0`，同时会话列表提示“Showing earlier chats”。该旧标签不能作为当前授权有效的证据。返回公网 `/user` →「WebIDE」→「刷新模型与权限状态」（可用模型 `glm-5.3.flash`，进度 25%）→「打开 MoonDesk / MoonCode」→ `:5003/connect` →「代码」，旧会话与历史消息重新加载；发送“请只回答：本轮恢复验收通过。”后页面先显示“运行中”，最终显示“DONE”及模型回答“本轮恢复验收通过。”。因此重新交接后的真实推理链通过。
8. 已修复旧直链的原始 JSON 解析错误：WebIDE 网关在 `/api/` 的未授权请求上保留 HTTP 401，同时返回 `application/json` 和明确的“从企业门户重新打开”消息；普通页面仍保留原来的纯文本保护。原生 WebIDE 专项测试 `15/15` 通过，源码提交 `3874131` 已推送 GitHub/GitLab `main`。新 amd64 网关镜像由该提交的干净归档构建，私有仓库 TLS 摘要回读为 `sha256:7ff37b5624fc2a60cbb39f51c99e045d0210860441486f0e85d5da9eb88658c1`；5002、5003 两个 Deployment 依次滚动完成。公网无登录 API 复测均返回 401 JSON；5003 旧浏览器标签再次发送后显示可操作的“Launch this workspace again from your portal.”，不再显示解析错误。重新从 `/user` 一键打开 5003，历史消息仍在，发送“请只回答：网关升级后仍可使用。”得到 `DONE` 和真实回复“网关升级后仍可使用。”。5005 ComfyUI 页面及视频工作流仍可打开。
