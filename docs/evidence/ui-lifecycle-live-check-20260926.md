...
| IaaS 报价权限 | 选择配置 → 填 Linux 用户名 `wangtest26` → 区域 `cn-north-1` → 24 小时 →「查看报价」；旧版本页面显示“当前账户无此操作权限” | 正确拒绝：wang 只有 Developer，缺少 `LeaseRequester`。没有签名报价、订单、付款或机器租约。新版已部署，但 GLM 恢复后两台 Spark 重新繁忙，本轮未再次停机触发新文案。 |
