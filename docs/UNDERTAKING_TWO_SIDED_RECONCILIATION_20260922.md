...
> **后续政策更正（2026-09-22）**：平台所有者已明确要求移除“双语法律模板”作为上线证据的硬性条件。
> 本文以下“双语/中英模板仍缺失”的记录保留为当时的历史判断，不再构成当前阻塞。
> 当前要求以 `docs/OFFLINE_COMMERCE.md` 为准：批准明确语言范围内的模板版本及不可变哈希，
> 保留原文/字段完整性、批准来源、合法字体与真实逐页渲染验收；旧模板未验收的字体不能因此放行。
> 具名第三方法务认证不是平台上线指令可以自动产生的事实，不得虚构姓名或认证。
...
所以第 3–6 步在真实集群上**纯 UI 走不到**；但**闸门打开之后这条链本身是通的** ——
§12.23 用端到端验收测试把第 1–7 步整条走完了（模板 → 报价 → begin → 生成 → 评审 → 释放 →
...
| 3 | 拉起承诺函 | ⛔ 不可达 | ✅ 通（模板注册 → 生成请求 → dispatcher 认领 → 终态回执 → 释放包） |
| 4 | 管理侧确认、执行 | ⛔ 不可达 | ✅ 通（报价 → begin → 评审 → 释放 → 履约） |
| 5 | 进入线下流程 | ⛔ 不可达 | ✅ 通（`PendingInternalApproval → PendingOfflineExecution → PendingEvidenceUpload`） |
| 6 | 重新上传录入登记 | ⛔ 不可达 | ✅ 通（五份上传授权 → 上传回调 → `UnderReconciliation` → 评审） |
| 7 | 开通权限 | ✅ 通（**真实身份**，`…:enable → 200`，§12.17） | ✅ 通（这一步走另一条路，不经过闸门） |
| 8 | 企业侧得到权限 | ⛔ **实际上没拿到**（堵点 J） | ⛔ **同样没拿到** |

**第 8 步是唯一两列都不通的**，而且它**与就绪闸门无关**：§12.23 那条链走的是
