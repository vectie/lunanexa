"""Build a lunanexa-identity-gateway image without docker.

The management node has no docker/podman/nerdctl/buildah, so
scripts/deploy/build-management-images.sh (docker build) cannot run there. This
is the ctr equivalent, derived from build-control-image.py with three changes:

  * the binary is installed as /usr/local/bin/lunanexa-identity-gateway
  * there is only one binary, so --aux-binary is gone
  * when no replaceable binary layer is found the new binaries are appended
    instead of failing, because layers stack last-wins and picking a layer to
    delete is only a size optimisation that depends on the base's layout

Two things bit us while getting this working, both recorded in
docs/IMAGE_BUILD_AND_DEPLOY_SOP.md section 5.1:

  * MOON_HOME must be the directory that carries the registry with the module
    versions you need (~/.moon), with the staged linux toolchain lent to it via
    bin/ and lib/ symlinks. Pointing MOON_HOME straight at a toolchain
    directory makes dependency resolution fail against that toolchain's older
    bundled registry.
  * Layer file lists must come from tarfile members with isfile(), not from
    getnames() minus trailing slashes: directory entries have no trailing slash
...
BINARY_NAMES = ("lunanexa-identity-gateway",)
...
def main():
    ...
    parser.add_argument("--binary", required=True, help="path to the built identity-gateway binary")
    ...
    for position, layer in enumerate(manifest["layers"]):
        with tarfile.open(f"{work}/base/blobs/sha256/{layer['digest'].split(':')[1]}", "r:*") as archive:
            # Directory entries are not files; counting them made every layer look
            # like it held something other than binaries.
            names = [member.name for member in archive.getmembers() if member.isfile()]
        if any("lunanexa-identity-gateway" in name for name in names) and is_binary_layer(names):
    ...
    if binary_layer is None:
        ...
        # delete is only a size optimisation and it depends on the base's layer
        # layout, so do not fail the build on it.
    ...
    with tarfile.open(fileobj=buffer, mode="w") as archive:
        for source, target in (
            (arguments.binary, "usr/local/bin/lunanexa-identity-gateway"),
        ):
    ...
    print(f"GATEWAY-IMAGE-OK {arguments.image} {archive_path}")
