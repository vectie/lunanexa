...
class OmniOpenAIServingVideo:
    ...
    async def generate_videos(
        ...
        *,
        ...
        # LunaNexa: MP4 encoding is CPU-bound and was running on the event loop.
        # For a 20 s clip it takes 102.7 s, during which the server answers
        # nothing at all -- including its own /health, which fails the readiness
        # probe, drops the pod out of its Service and returns 502 to every other
        # caller. Move it to a worker thread so the loop keeps serving.
        encoded: list[str] = []
        ...
        video_data = [
            VideoData(b64_json=encoded[idx], action=artifacts.actions[idx])
