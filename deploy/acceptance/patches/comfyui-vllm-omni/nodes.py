    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                # LunaNexa: upstream defaults this to 16, which the MiniMax-H3
                # service this cluster actually serves rejects outright --
                # pipeline_minimax_h3._resolve_shape raises "MiniMax H3 output fps
                # is fixed at 24" and the request dies as an HTTP 500. A fresh
                # canvas therefore failed before it could ever work, and the
                # failure surfaced as an unrelated message. 24 is the only value
                # H3 accepts; other vLLM-Omni video models are unaffected by
                # leaving it at their own default, which the operator can set.
