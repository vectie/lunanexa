...
async def run_video_job(
    ...
    * The completed-video download does not inherit the control-plane timeout.
      The service encodes the MP4 while the request is open (102.7 s for a 20 s
      clip), so a 60 s session budget aborts a job that has already finished.
      The enclosing asyncio.timeout(max_poll_duration) still bounds the job.
    * A job that reached `completed` but whose bytes never reached the caller is
    ...
    download_timeout = aiohttp.ClientTimeout(total=None, sock_connect=30, sock_read=None)
    job_id = None
    completed = False
    delivered = False
        ...
        try:
            ...
            if job_id is not None:
                if completed and not delivered:
                    # The clip exists but its bytes never reached the caller;
                    # deleting the job would destroy the only copy.
                    logger.warning("Completed video was not delivered; keeping the job for retrieval")
                else:
                    try:
